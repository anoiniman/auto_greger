...
function debug_move(dir, distance, forget)
...
function set_up_navigate_chunk(what_chunk)
    cur_in_road = false
...
function navigate_chunk(what_kind)
    ...
    -- still haven't arrived at target chunk
    -- TODO WRITE THIS
...
function real_move(what_kind, direction)
...
function base_move(direction) -- return result and error string
...
function change_orientation(goal) 
...
function convert_orientation(o_orient)
...
function update_pos(direction) -- assuming forward move
...
function mark_chunk(what_chunk, as_what)
